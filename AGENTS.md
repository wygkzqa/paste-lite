...
- `AppSettings` 保存剪贴板布局（默认列表，可切换卡片）、渐变背景开关（默认关闭）及语言偏好，语言默认跟随系统；`L10n` 统一读取 `en.lproj` / `zh-Hans.lproj` 文案。界面语言切换不得重写历史、用户内容或筛选值。
...
- 两种布局共用 `ClipboardGlass`：macOS 26 使用原生 Liquid Glass，旧版回退半透明材质，“减少透明度”时使用不透明背景。可选蓝紫渐变只绘制于面板底层，适配系统明暗；卡片横向、列表纵向使用惰性视图和同一分页数据，不为每条记录添加玻璃效果。
- 视图负责展示与交互回调；存储、权限请求和系统粘贴行为放在现有服务或控制器中。
- 设置使用通用、历史、数据、关于四个侧栏分类，语言/布局/开机启动及文本和图片采集大小限制归入通用；菜单栏的关于入口通过同一个 `SettingsNavigation` 切换至设置中的关于页，不创建独立关于窗口。清空历史须由用户在界面确认；仅删除历史、图片、缩略图和旧迁移备份，保留分组、设置、系统当前剪贴板及外部原文件。测试仅使用隔离数据库与命名剪贴板。
...
- ⇧⌘V 打开或关闭列表；单击仅选择，双击或 Return 执行复制并尝试粘贴。
