...
- `AppUpdateManager` 共用一个 Sparkle 更新器，菜单栏与关于页使用同一状态；`AppUpdateView` 沿用应用语言与外观。自动检查默认关闭，下载和安装需用户操作；缺少正式公钥时明确禁用更新。安装前检查编辑与导入窗口，退出前停止采集并等待存储队列完成；取消准备好的更新不能变成退出时安装。SPM 固定版本和锁文件，正式清单与最终 DMG 均须验签，私钥只在受保护的标签发布步骤使用。
